...
## 전략 성적이 나쁠 때 — 진입인가 청산인가부터 가른다

백테스트 성적이 나쁘면 원인은 둘 중 하나다. **진입 신호에 우위가 없거나**,
**청산 규칙이 신호를 죽이거나.** 둘은 정반대의 대응을 요구한다. 구분하지 않고
손절 폭을 이리저리 바꿔보는 것이 과최적화로 가는 가장 흔한 길이다.

    python -m autotrader --csv data/kiwoom --threshold 0.45 edge --bars 2500

`edge` 는 손절·목표가를 모두 끄고 신호 뒤 N일 수익률만 잰다. 같은 종목·같은
기간의 **모든 봉**을 기준선으로 삼아 그 차이만 우위로 본다 (시장이 그냥 오른
것과 구분하기 위해).

읽는 법:

- **t값 ≥ 2** 면 우위가 유의하다. 표본이 작으면 우위가 커 보여도 t 는 낮다.
- **점수 구간별 수익**이 단조 증가하지 않으면 앙상블 점수로 포지션 크기를
  조절할 근거가 없다.
- **역행 비율**이 손절 폭보다 크면 이길 자리에서 잘린다.

`--min-t 2.0` 을 주면 종료코드로 게이트가 된다.
