...
### 저회전 팩터도 폐기했다 (2026-08-26) — 이 fold 탐색 종료

`docs/LOW-TURNOVER-SPEC.md` 를 사전등록하고 실데이터로 돌린 결과도 **FAIL**
이다. 근거는 `docs/LOW-TURNOVER-RESULTS-2026-08-26.md`.

이번 실패는 앞의 것들과 성격이 다르다. **설계는 전부 의도대로 작동했다** —
회전율 1.35배(상한 2.0), 비용 연 0.32%, MDD -10.7%, Deflated Sharpe 0.99.
6개 기준 중 5개를 통과하고 **결정적인 하나에서 졌다**:

```
유니버스를 그냥 동일가중으로 사서 들고 있으면   연 +4.07%
저변동성으로 25종목을 고르면                  연 +2.13%
→ 종목 선택이 연 2%p 를 까먹었다
```

자본 제약이 원인인지 확인하려고 자본을 5배로 올려 다시 돌렸다. 예산초과가
4개→0, 보유가 항상 25종목으로 채워졌는데 **초과수익 개선은 1.68%p 뿐**이었다.
자본 규모는 원인이 아니고 **신호 자체가 가치를 더하지 못한다.**

그리고 가장 아픈 비교: 유니버스를 **절반만 사고 나머지를 현금**으로 두면
연 +2.04% 가 나온다 — 회전율도 비용도 없이. 변동성을 낮추는 것이 목적이었다면
종목을 고를 이유가 없었다.

**그래서 이 데이터셋(2016~2026, 4개 fold)에 대한 탐색은 끝났다.** 누적 5회
참조했다. 보유 종목 수·리밸런싱 주기·변동성 창·밴딩 폭을 바꿔 재시험하지
않는다 — 여섯 번째 참조가 되고, Bailey et al. 이 경고한 지점 그대로다.

새로운 시도는 **새 데이터로만** 한다: 최소 60거래일 모의투자, 또는 이 기간
밖의 미사용 구간.

정직한 다음 행동은 "다른 전략 찾기" 가 아니라 **저비용 지수 투자 + ISA 세제
활용**이다 (`docs/STRATEGY-RESET-2026-08-26.md` §8-2).

### 외부 전략 제안을 받았을 때

폐기한 5종을 다른 이름으로 다시 제안하는 문서가 온다. 잘 쓰였고 업계 통념과
일치하므로 그럴듯하다. `docs/PROPOSAL-REVIEW-2026-08-26.md` §4 의 7단계
체크리스트를 순서대로 적용한다. 앞의 세 개만으로 대부분 걸러진다:

1. **이미 시험했는가** — `autotrader/strategy/` 와 walk-forward 결과부터 본다
2. **연 회전율 × 왕복비용 = 연 비용 부담** — 12배(월 편도 50%)를 넘으면 탈락
3. **거래당 33bp 이상의 순수 우위를 낼 새 정보원이 있는가** — 기존 가격
   데이터를 다르게 조합하는 것뿐이면 아니다

벤더 마케팅은 1차 출처로 확인하기 전까지 근거가 아니다 (젠포트의 "엄격한
슬리피지 차감" 은 자체 매뉴얼에서 **선택 항목**이었다).
