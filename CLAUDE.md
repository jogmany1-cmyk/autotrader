...
### 폐기된 전략군 (2026-08-26) — 되살리지 말 것

일봉 기술적 지표 5종(`DayBreakout` · `DayPullback` · `DayMomentum` ·
`SwingTrend` · `MeanReversion`)과 그 파생 실험(`swing_trend_v2_experimental`)은
**전부 폐기**했다. 근거는 `docs/STRATEGY-RESET-2026-08-26.md`.

핵심 숫자 하나만 기억하면 된다:

```
거래당 순수 우위(비용 전)  -27.9bp ~ +8.0bp
필요한 왕복비용            33bp(코스피 대형) ~ 103bp(코스닥 중소형 시장가)
                           그 중 20bp 는 증권거래세 — 협상 불가
```

**지표를 바꿔서 메울 수 있는 격차가 아니다.** 일 단위 매매로 수익을 내려면
거래당 최소 33bp 의 순수 우위가 필요하고, 실제로는 그 2~3배가 나와야 한다.

그래서 다음을 하지 말 것:

- 폐기한 5종에 임계값·손절폭·보유기간을 바꿔 재시험하는 것. 같은 4개 OOS
  fold 를 이미 **네 번** 참조했다. Bailey et al. (AMS 2014): 과최적화된
  전략의 표본 외 기대수익은 0 이 아니라 **음수**다.
- 뉴스·공시를 **알파(수익 예측)** 로 쓰는 것. 호재는 1분 내 완전 반영되고
  (Busse & Green, JFE 2002), 묵은 뉴스 매매는 개인 과잉반응 패턴으로 다음 주에
  반전된다 (Tetlock, RFS 2011). 뉴스는 `intelligence/veto.py` 를 통한
  **RiskEngine 거부권(수비)** 으로만 쓴다.
- 한국 시장에서 **모멘텀/추세추종**을 기본 가정으로 두는 것.
  Chui·Titman·Wei (JF 2010) 에서 한국·중국·일본 모두 유의하지 않았다.

현재 유효한 방향은 `docs/LOW-TURNOVER-SPEC.md` — 반기 리밸런싱, 25종목
동일가중, 연 회전율 ≤ 2배. **이 규격도 실패하면 폐기하고, 임계값을 돌려가며
통과 조합을 찾지 않는다.**
