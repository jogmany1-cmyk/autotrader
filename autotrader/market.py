...
# ---- 호가단위(틱) 표 -------------------------------------------------------
#
# KRX 는 2023-01-25 에 호가단위를 통일·축소했다 (약 12년 만의 변경).
# 슬리피지를 고정 bp 로 두면 이 구조가 통째로 무시된다. 틱 하나가 **가격의
# 몇 bp 인가**는 가격대에 따라 크게 달라지기 때문이다. 구간 **하단**이 가장
# 나쁘다 — 틱이 막 커진 직후이기 때문이다:
#
#   1,999원 → 1원 틱 =  5.0bp   |   2,000원 → 5원 틱 = 25.0bp
#   4,900원 → 5원 틱 = 10.2bp   |   5,000원 → 10원 틱 = 20.0bp
#
# 즉 2,000원짜리 종목은 1틱만 넘겨도 편도 25bp 다. 고정 5bp 가정의 5배다.
#
# 왜 이것이 중요한가: 백테스터는 **다음 봉 시가에 체결**한다. 그것은 시장가
# 주문이고, 시장가는 스프레드를 넘어가야 한다. 편도 5bp 고정은 사실상
# "항상 중간가에 체결된다"를 모델링한 것이며, 실제로는 도달 불가능하다.
# (Barber·Lee·Liu·Odean, RFS 2009: 개인 손실의 사실상 전부가 공격적 주문에서
#  나온다.)
#
# (상한 미만, 틱). 마지막 구간(50만원 이상)은 1,000원.
KRX_TICK_TABLE = (
    (2_000, 1),
    (5_000, 5),
    (20_000, 10),
...
KRX_TICK_ABOVE_TOP = 1_000


def krx_tick_size(price: float) -> int:
    """이 가격에서의 호가단위(원). 가격이 0 이하면 최소 틱을 돌려준다."""
    if price <= 0:
        return KRX_TICK_TABLE[0][1]
    for upper, tick in KRX_TICK_TABLE:
        if price < upper:
            return tick
    return KRX_TICK_ABOVE_TOP


def krx_tick_bp(price: float) -> float:
    """호가단위 하나가 이 가격의 몇 bp 인가.

    저가주일수록 상대적 틱이 커진다 — 그래서 같은 "1틱 넘김"이라도 저가
    코스닥 종목에서 훨씬 비싸다. KCMI(2024) 호가단위 축소 연구는 저가 ETP 의
    최소 호가스프레드가 틱을 1원으로 낮추자 1.4% → 0.28% 로 떨어진 것을
    측정했다 — 상대 틱 크기가 얼마나 지배적인지 보여준다.
    """
    if price <= 0:
        return 0.0
    return krx_tick_size(price) / price * 10_000.0
